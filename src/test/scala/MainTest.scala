...
class calculateStatsTest extends FunSuite:
  test(
    "getStreamingProvider should return an array of unique streaming providers"
  ) {
    val countries = Array(
  ...
  test(
    "getSpreadStreamingProvider should return a map of streaming providers and their percentage of countries"
  ) {
    val countries = Array(
      Country("Country1", "DE", List("Netflix", "Hulu")),
      Country("Country2", "CH", List("Netflix", "Amazon Prime")),
